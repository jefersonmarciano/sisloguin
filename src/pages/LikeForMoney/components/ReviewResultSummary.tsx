  ...
  return (
    <div className={cn("bg-white rounded-lg shadow-md p-4 border border-gray-100", className)}>
      <div className="flex justify-between items-center mb-3">
        <h2 className="font-bold text-lg">Current Session Results</h2>
          ...
          {results.map((result, index) => (
            <div 
              ...
              className={cn(
                ...
                result.type === 'like' ? "bg-green-100" : 
                result.type === 'dislike' ? "bg-amber-100" : "bg-gray-100"
              )}
            >
              {result.type === 'like' ? (
      ...
      <div className="grid grid-cols-3 gap-2 text-sm">
        <div className="flex items-center gap-1">
          ...
          <span className="text-gray-700">Likes: {likeCount}</span>
        </div>
        <div className="flex items-center gap-1">
          <ThumbsDown size={14} className="text-amber-600" />
          <span className="text-gray-700">Dislikes: {dislikeCount}</span>
        </div>
        <div className="flex items-center gap-1">
          <CircleX size={14} className="text-gray-600" />
          <span className="text-gray-700">Skipped: {skipCount}</span>
        </div>
      </div>
    </div>
  );
