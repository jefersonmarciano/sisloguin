...
const SessionSummary: React.FC<SessionSummaryProps> = ({ results, onContinue }) => {
  ...
  return (
    ...
    >
      ...
      <h2 className="text-2xl font-bold mb-2 text-gray-800">{t('sessionComplete')}</h2>
      ...
      <div className="bg-gradient-to-r from-temu-orange/10 to-amber-500/10 rounded-lg p-6 w-full max-w-md mb-8">
        <div className="flex justify-between items-center mb-4">
          <span className="font-medium text-gray-700">{t('totalEarned')}</span>
          <span className="text-xl font-bold text-green-600">${totalEarned.toFixed(2)}</span>
        </div>
        
        <div className="space-y-3">
          <div className="flex justify-between items-center">
            <span className="font-medium text-gray-700">{t('inspections')}</span>
            <span className="font-semibold">{results.length} {t('completed')}</span>
          </div>
          
          <div className="flex justify-between items-center">
            <span className="font-medium text-gray-700">{t('correctAssessments')}</span>
            <span className="font-semibold flex items-center gap-1">
              {correctAssessments} <Check size={16} className="text-green-500" />
            </span>
          </div>
          
          <div className="flex justify-between items-center">
            <span className="font-medium text-gray-700">{t('incorrectAssessments')}</span>
            <span className="font-semibold flex items-center gap-1">
              {results.length - correctAssessments} <X size={16} className="text-red-500" />
            </span>
          </div>
          
          <div className="flex justify-between items-center">
            <span className="font-medium text-gray-700">{t('accuracyRate')}</span>
